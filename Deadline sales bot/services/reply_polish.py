...
# Голый ответ-имя на вопрос бота «как вас зовут?» — частые НЕ-имена (отказы,
# подтверждения, дни/время/каналы), которые нельзя принять за имя.
_BARE_NAME_REJECT = {
    "да", "нет", "ок", "окей", "окк", "ага", "угу", "хорошо", "спасибо", "привет",
    "здравствуйте", "понятно", "конечно", "можно", "нельзя", "завтра", "сегодня",
    "потом", "позже", "сейчас", "давайте", "давай", "утром", "вечером", "днем",
    "днём", "ночью", "пятница", "понедельник", "вторник", "среда", "четверг",
    "суббота", "воскресенье", "телеге", "телеграм", "телега", "ватсап", "вотсап",
    "зум", "знаю", "думаю", "скажу", "ничего", "нормально", "норм", "всё", "все",
    "сайт", "магазин", "бот", "лендинг", "цена", "цены", "сколько", "что", "как",
}
_BARE_NAME_TOKEN = re.compile(r"^[A-Za-zА-Яа-яЁё]{2,20}$")


def mentions_name_ask(text):
    """True, если сообщение бота содержит вопрос про имя («как вас зовут» и т.п.)."""
    t = (text or "").lower()
    return any(p in t for p in NAME_ASK)


def looks_like_bare_name(text):
    """Голый ответ-имя (1–2 слова, только буквы, без явных не-имён). Звать ТОЛЬКО
    если прошлым ходом бот спросил имя (mentions_name_ask) — иначе примем за имя
    «Картошка»/«Сайт». Возвращает нормализованное имя или None."""
    if not text:
        return None
    ...
    if not (1 <= len(toks) <= 2):
        return None
    for w in toks:
        if not _BARE_NAME_TOKEN.match(w):
            return None
        if w.lower() in _BARE_NAME_REJECT or w.lower() in _NAME_STOP:
            return None
    return " ".join(w[:1].upper() + w[1:].lower() for w in toks)
