...
def parse_slot_choice(
    ...
    if len(matched) == 1:
        return matched[0]
    if len(matched) >= 2:
        # Один названный час совпал с НЕСКОЛЬКИМИ слотами — это бывает, когда
        # слоты предложены в один час в РАЗНЫЕ дни (напр. «среда 15:00» и
        # «четверг 15:00»). Если лид назвал ещё и ДЕНЬ («в среду в 15») — он
        # однозначно разрешает выбор. Раньше тут был return None → «в среду в 15»
        # не бронировалось, хотя среда 15:00 ровно из предложенных.
        by_day_among = _match_offered_by_day(t, matched, now_utc)
        if by_day_among is not None:
            return by_day_among
        return None  # назвал несколько времён без дня → выбор не однозначен
