...
│
├── 💬 telegram: @ivan_petrov                            ← topic создан, ЗАКРЫТ для ввода
│   ├── 📥 LEAD · @ivan_petrov
│   │   привет
│   ├── 📤 BOT
│   │   Привет. Я — AI-агент Deadline... [👤 Возьму на себя]
│   ├── 📥 LEAD · @ivan_petrov  🎙️ 18s                      ← голосовое
│   │   нужен сайт под кофейню
│   ├── 📤 BOT
│   │   Понятно. Лендинг кофейни... [👤 Возьму на себя]
│   ↓
│   (оператор нажал кнопку — топик ОТКРЫЛСЯ, поле ввода появилось)
│   🔔 OPERATOR TAKEOVER ON
│   ✍️ Иван, привет — давай созвонимся в понедельник?      ← оператор пишет
│   ✅ Доставлено лиду. [🤖 Вернуть боту]                  ← подтверждение от бота
│   ↓
...
1. Открой бота `@Deadline_Corp_bot` в Telegram (или попроси кого-то ещё не из команды)
2. Напиши: `привет`
3. В группе `Deadline · Sales Inbox` появится новая тема `telegram: <username>` (она сразу **ЗАКРЫТА** для ввода — это нормально, операторы должны только наблюдать пока бот говорит) с зеркалом:
   ```
   📥 LEAD · @username
   привет
...
2. В теме появится транскрипт с пометкой 🎙️ и длительностью:
   ```
   📥 LEAD · @username  🎙️ 12s
   нужен сайт для кофейни с меню и контактами
...
2. Тема **открывается** для ввода (поле появляется), в шапке `🔔 OPERATOR TAKEOVER ON`
...
5. Под каждым твоим сообщением бот ставит подтверждение `✅ Доставлено лиду.` с кнопкой **`🤖 Вернуть боту`**
6. Лид отвечает → его сообщение приходит в тему как `📥 LEAD`, бот **не отвечает сам**
7. Оператор продолжает писать — каждая реплика идёт лиду
8. **Чтобы вернуть диалог боту**: жми кнопку `🤖 Вернуть боту` (или пиши `/release` — то же самое). Тема снова закрывается для ввода, бот отвечает сам.

**Дополнительно**: оператор может слать **голос/фото/документ/видео/стикеры** в тему — бот пересылает их лиду в Telegram через тот же mechanism. Лид получает media с подписью (caption) если она была. Для IG/FB attachment forwarding пока не поддержан (нужна отдельная upload-схема Meta) — там пиши только текстом.
...
|---|---|---|
| Telegram | весь диалог + voice + любые attachments | через Bot API Send (instant) |
| Instagram DM | весь диалог + voice transcripts | через Graph API Send API c `messaging_type=MESSAGE_TAG, tag=HUMAN_AGENT` — работает в окне 7 дней после последнего сообщения лида |
| Messenger | весь диалог | через Graph API Send API c `tag=HUMAN_AGENT` — окно 7 дней |
...
Для IG/Messenger: `HUMAN_AGENT` tag требует permission `human_agent` на Meta App (Standard Access в App Review). После одобрения операторы могут отвечать лидам до 7 дней с момента их последнего сообщения, не только в стандартном 24-часовом окне.
...
- ❌ **Не кидай attachments если лид в IG/FB/website** — для Telegram-лидов attachment forwarding работает (voice, photo, video, document, sticker, animation), для остальных каналов только текст. На IG/FB attachment'ы requires отдельный `/me/message_attachments` upload (deferred). Если оператор пытается отправить attachment IG/FB лиду — бот логирует warning и НЕ доставляет, нужно переслать текстом.
