...
| [duplicate-prevention.md](duplicate-prevention.md) | Защита от дублей ВСЕХ каналов: идемпотентность приёма + эхо-claim + канал-независимый дедуп по message-id + кнопка «Синхронизировать» (одно окно) | `services/whatsapp_sync.py` (dedup_messages_global, cleanup_wa_artifacts), `main.py`, `admin_api.py` (/maintenance/dedup) |
