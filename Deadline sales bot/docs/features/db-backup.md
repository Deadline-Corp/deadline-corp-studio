...
## Экспорт в таблицу (Excel / CSV) — Фаза 0.2

Помимо логического бэкапа (для восстановления) есть человекочитаемые выгрузки «на всякий
случай», открываемые в Excel / Google Sheets (UTF-8 BOM, разделитель `;` под русскую локаль):

| Эндпоинт | Что | Кто |
|---|---|---|
| `GET /admin/api/export/leads.csv` | CRM-таблица лидов: имя, email, телефон, канал, стадия, причина проигрыша, скор, температура, даты + значения кастом-полей | member |
| `GET /admin/api/export/conversations.csv` | ПОЛНЫЙ архив переписок: каждое сообщение строкой (Лид, Телефон, Канал, Стадия, Время, Кто=Лид/Бот/Менеджер, Текст), вкл. архивные карточки. Потолок 50 000 сообщений | member |

UI: **Настройки → «📊 Экспорт в таблицу»** (кнопки «👤 Лиды (CRM)» и «💬 Все переписки»).
Код: `admin_api.py` → `export_leads_csv`, `export_conversations_csv`.

---

## Ребайнд WhatsApp — смена номера бота без потери базы клиентов

**База клиентов привязана к ТЕЛЕФОНУ ЛИДА, а не к номеру/сессии бота.** Идентичность —
`ChannelIdentity(channel='whatsapp', external_id=<телефон лида>)`, диалоги после phone-canonical
фикса ключуются телефоном лида (`conversations.channel_conversation_id`). Номер самого бота —
это лишь связанное устройство WAHA-сессии (`WAHA_BASE_URL` + QR), он НЕ хранится в карточках лидов.

**Поэтому при смене номера бота (отвалился WhatsApp / нужен другой номер):**
1. Поднять/перенаправить WAHA на новый номер (новый QR-скан в WAHA), `WAHA_BASE_URL` тот же.
2. База (customers/conversations/messages) остаётся целой — ничего мигрировать не нужно.
3. Входящие от тех же лидов на новый номер бота резолвятся в ТОТ ЖЕ customer (по телефону лида).

Запасной номер для теста: `+66967239484`. Перед сменой — снять `/db-backup` (offsite) и
снимок конфигурации (Настройки → «🗂 Версии конфигурации»).
