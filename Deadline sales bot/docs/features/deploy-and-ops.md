...
- **Дневной потолок исходящих** (`main._wa_over_daily_cap`, env `WA_DAILY_SEND_CAP`,
  дефолт **300/сутки**, 0 = без лимита): при достижении `_wa_send` пропускает отправку
  (лог-предупреждение) — предохранитель от убегания автопилота/массового всплеска.
...
## Закалка /health (инцидент-фикс 06-16)
- Раньше `check_connection()` звался СИНХРОННО на event-loop и делал checkout из пула;
  при исчерпанном пуле висел → `/health` не отвечал, хотя loop был жив (статика/404 ОК).
  Railway решает рестарт ПО `/health` → если он висит, рестарта нет, вотчдог —
  единственная страховка.
- Теперь DB-проверка уходит в поток + жёсткий таймаут 3с: `/health` **всегда** отвечает
  быстро 200/`ok:true`; при медленной/недоступной БД → `db_connected:false` (degraded),
  но не вис.

## Дедуп @lid-дублей по телефону
Рекламные лиды приходят под скрытым `@lid` и заводят свою карточку; на входящем телефон
резолвится (WAHA LID API) и штампуется на `customer.phone`.
- **Авто (cron, каждый цикл, только БД)**: `dedup_wa_by_phone()` — если 2+ активные
  whatsapp-карточки имеют один штампованный телефон, старую → ARCHIVED (обратимо),
  активную (где живые сообщения) оставляет. Переносит стадию (вперёд)/черновик/summary.
- **Бэклог (вручную)**: `POST /admin/api/whatsapp/dedup {execute:true, resolve_lids:true, limit:N}`
  — добивает телефоны у старых @lid-карточек (СЕТЬ, bounded, commit перед сетью) → дедуп
  их схлопывает. Ответ: `resolved_lids`, `phone_dedup`.
