...
**Массовый пинок (Фаза 3, 2026-06-16):** «📣 Массовый пинок» — ОДИН текст всем видимым молчунам
с подстановкой `{name}`/`{имя}` (мягкая персонализация против «одинаковости»). `POST /whatsapp/mass-nudge`
`{text, conversation_ids}` → фон `_run_mass_nudge_bg` (тот же троттл + `_WA_SEND_STATE`). Оператор
пишет/правит текст, подтверждает, шлёт. ⚠️ Идентичная рассылка рискованнее персональных черновиков —
поэтому имя подставляется + рекомендация «короткий нейтральный вопрос, не спам».
