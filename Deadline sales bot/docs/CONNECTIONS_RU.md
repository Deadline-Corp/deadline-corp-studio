...
| `stuck` | активные, не разобранные, молчат >48ч | блок «🆘 Бот завис — помоги» с «💬 Объяснить боту» (рендерится; исключены из «Без задачи», чтобы не задвоить) |
| `deal_value` | `conversations.deal_value` | чип суммы сделки на карточках (₸К/₸М, крупные ≥500К зелёным) |
...
| 💬 Объяснить боту (блок «Затыки») | POST `/conversations/{id}/rethink` `{hint}` | бот ПЕРЕразбирает лид с подсказкой менеджера → готовит шаг/черновик. Лиду НИЧЕГО не шлёт. Гард `_verify_member` |
...
+ **`stuck` (блок «🆘 Бот завис»)** + `delivery_failed`. А вот
`zones.{your_turn,bot_leading,waiting}` бэкенд считает и отдаёт, но дневная доска их
**не выводит** (компонент `Zone` определён, но не вызывается). Лиды из этих зон не
теряются — они попадают в `buckets` (если есть задача) или в `no_task_leads`/`stuck`
(если нет). Это не баг, а выбор UX (доска организована «по дням», а не «по зонам» —
иначе те же лиды дублировались бы). Данные зон есть, при желании рендер добавляется.
...
## 10. Бэклог усиления задачника — РЕАЛИЗОВАНО 2026-06-19

| # | Улучшение | Статус |
|---|---|---|
| 1 | Блок «🆘 Бот завис — помоги» (`stuck`) + «💬 Объяснить боту» (переразбор с подсказкой) | ✅ сделано |
| 2 | Фильтр по стадии применяется к «Затыкам» и «Одобрить» (не только buckets) | ✅ сделано |
| 3 | Таблетка «🏷 Без задачи N» в строке фильтров (фокус-режим) | ✅ сделано |
| 4 | Инлайн-просмотр полного черновика в approve_now (клик по тексту, без Drawer) | ✅ сделано |
| 5 | Метка «обновлено N сек назад» + жёлтое предупреждение при >60с | ✅ сделано |
| 6 | `deal_value` чипом (₸К/₸М) на всех карточках | ✅ сделано |
| 7 | «через 2ч» рядом со временем «когда бот напишет» (bot-задачи) | ✅ сделано |
| 8 | «✓ Сделано» для лида в your_turn без task_id | ⏸ N/A — зона your_turn в дневной доске не рендерится (такие лиды видны как задачи/в «Без задачи») |

**Новая связь «Объясни боту»:** карточка «Затыка» → поле подсказки → POST
`/conversations/{id}/rethink` `{hint}` → `generate_next_action(hint=...)` подмешивает
подсказку в LLM-промпт → бот выдаёт `next_action` (шаг/черновик), подсказка пишется в
`next_action.hint`. После переразбора лид помечен `analyzed=True` и уходит из «Затыков»
в нужный блок (одобрение / задача / ожидание). Лиду ничего не отправляется.
